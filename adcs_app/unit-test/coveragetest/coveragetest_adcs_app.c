...
/*
** File: coveragetest_adcs_app.c
**
** Purpose:
** Coverage Unit Test cases for the ADCS Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the ADCS application.
**
...
#include "adcs_lib.h" /* For ADCS_LIB_Function */
...
void Test_ADCS_APP_Main(void)
{
    ...
    /*
     * ADCS_APP_Main does not return a value,
     ...
     * The first is for ADCS_APP_Init().  As this is in the same
     ...
     * the ADCS_APP_Init() failure path was taken.
     */
    ADCS_APP_Main();
     ...
     */
    UtAssert_UINT32_EQ(ADCS_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_PIPE_ERR_EID, "ADCS APP: SB Pipe Read Error, App Will Exit");
...
void Test_ADCS_APP_Init(void)
{
    ...
    UtAssert_INT32_EQ(ADCS_APP_Init(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(ADCS_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(ADCS_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(ADCS_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 5);
}

void Test_ADCS_APP_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void ADCS_APP_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        ADCS_APP_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_INVALID_MSGID_ERR_EID, "ADCS: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(ADCS_APP_CMD_MID);
    FcnCode   = ADCS_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    ADCS_APP_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(ADCS_APP_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    ADCS_APP_ProcessCommandPacket(&TestMsg.SBBuf);
...
void Test_ADCS_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void ADCS_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        ADCS_APP_NoopCmd_t          Noop;
        ADCS_APP_ResetCountersCmd_t Reset;
        ADCS_APP_ProcessCmd_t       Process;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_COMMANDNOP_INF_EID, NULL);

    ADCS_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_UINT32_EQ(EventTest.MatchCount, 1);

    /* test dispatch of RESET */
    FcnCode = ADCS_APP_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_COMMANDRST_INF_EID, NULL);
    ...
    /* note this will end up calling ADCS_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = ADCS_APP_PROCESS_CC;
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_COMMAND_ERR_EID, "Invalid ground command code: CC = %d");
    ADCS_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_ADCS_APP_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void ADCS_APP_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(ADCS_APP_SEND_HK_MID);

    /* Set message id to return so ADCS_APP_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    ADCS_APP_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_STUB_COUNT(CFE_SB_TransmitMsg, 1);
    UtAssert_ADDRESS_EQ(MsgSend, &ADCS_APP_Data.HkTlm);

    /* Confirm timestamp msg address */
    UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
    UtAssert_ADDRESS_EQ(MsgTimestamp, &ADCS_APP_Data.HkTlm);
...
void Test_ADCS_APP_NoopCmd(void)
{
    /*
     * Test Case For:
     * void ADCS_APP_NoopCmd( const ADCS_APP_Noop_t *Msg )
     */
    ADCS_APP_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;
    ...
    UtAssert_INT32_EQ(ADCS_APP_Noop(&TestMsg), CFE_SUCCESS);
...
void Test_ADCS_APP_ResetCounters(void)
{
    /*
     * Test Case For:
     * void ADCS_APP_ResetCounters( const ADCS_APP_ResetCounters_t *Msg )
     */
    ADCS_APP_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_COMMANDRST_INF_EID, "ADCS: RESET command");

    UtAssert_INT32_EQ(ADCS_APP_ResetCounters(&TestMsg), CFE_SUCCESS);
...
void Test_ADCS_APP_ProcessCC(void)
{
    /*
     * Test Case For:
     * void  ADCS_APP_ProcessCC( const ADCS_APP_Process_t *Msg )
     */
    ADCS_APP_ProcessCmd_t TestMsg;
    ADCS_APP_Table_t      TestTblData;
    void *                  TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the ADCS_APP_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UtAssert_INT32_EQ(ADCS_APP_Process(&TestMsg), CFE_SUCCESS);
    ...
    /*
     * Confirm that the ADCS_LIB_Function() call was done
     * NOTE: This stub is provided by the adcs_lib library
     */
    UtAssert_STUB_COUNT(ADCS_LIB_Function, 1);
     ...
     */
    UT_SetDefaultReturnValue(UT_KEY(CFE_TBL_GetAddress), CFE_TBL_ERR_UNREGISTERED);
    UtAssert_INT32_EQ(ADCS_APP_Process(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
}

void Test_ADCS_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool ADCS_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CHECKEVENT_SETUP(&EventTest, ADCS_APP_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    ADCS_APP_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    ADCS_APP_VerifyCmdLength(NULL, size + 1);
...
void Test_ADCS_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(ADCS_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return ADCS_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + ADCS_APP_TBL_ELEMENT_1_MAX;
    UtAssert_INT32_EQ(ADCS_APP_TblValidationFunc(&TestTblData), ADCS_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_ADCS_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(ADCS_APP_Main);
    ADD_TEST(ADCS_APP_Init);
    ADD_TEST(ADCS_APP_ProcessCommandPacket);
    ADD_TEST(ADCS_APP_ProcessGroundCommand);
    ADD_TEST(ADCS_APP_ReportHousekeeping);
    ADD_TEST(ADCS_APP_NoopCmd);
    ADD_TEST(ADCS_APP_ResetCounters);
    ADD_TEST(ADCS_APP_ProcessCC);
    ADD_TEST(ADCS_APP_VerifyCmdLength);
    ADD_TEST(ADCS_APP_TblValidationFunc);
    ADD_TEST(ADCS_APP_GetCrc);
}
